...
from Gbot import QUEEN_PTB, DEV_USERS, telethn, OWNER_ID
...
async def restart(update: Update):
    ...
    process = subprocess.run("pkill python3 && python3 -m Gbot",
                             shell=True,
                             check=True)
    process.communicate()


QUEEN_PTB.add_handler(CommandHandler("install", pip_install, block=False))
QUEEN_PTB.add_handler(CommandHandler("leave", leave, block=False))
QUEEN_PTB.add_handler(CommandHandler("gitpull", gitpull, block=False))
QUEEN_PTB.add_handler(CommandHandler("reboot", restart, block=False))
QUEEN_PTB.add_handler(CommandHandler("lockdown", allow_groups, block=False))
