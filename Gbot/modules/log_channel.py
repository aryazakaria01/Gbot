...
if is_module_loaded(FILENAME):
    ...
    from Gbot import GBAN_LOGS
    from Gbot.modules.helper_funcs.chat_status import (
        user_admin as u_admin,
        is_user_admin,
    )
    from Gbot.modules.sql import log_channel_sql as sql
    ...
    def __chat_settings__(chat_id, user_id):
        if log_channel := sql.get_chat_log_channel(chat_id):
            log_channel_info = asyncio.get_running_loop().run_until_complete(
                QUEEN_PTB.bot.get_chat(log_channel))
...
async def log_setting_callback(update: Update,
    ...
    QUEEN_PTB.add_handler(CommandHandler("logchannel", logging, block=False))
