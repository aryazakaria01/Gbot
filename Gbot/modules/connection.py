...
import Gbot.modules.sql.connection_sql as sql
from Gbot import QUEEN_PTB, SUDO_USERS, DEV_USERS
from Gbot.modules.helper_funcs import admin_status
...
async def connection_chat(update: Update,
    ...
    if conn:
        ...
        chat_name = await QUEEN_PTB.bot.getChat(conn).title
    else:
    ...
    if update.effective_chat.type == "private":
        if args and len(args) >= 1:
            ...
            if (isadmin) or (isallow and ismember) or (user.id in SUDO_USERS):
                if connection_status := sql.connect(
                    ...
                    conn_chat = await QUEEN_PTB.bot.getChat(await connected(
            ...
            conn = await connected(context.bot,
                                   ...
                                   need_admin=False)
            if conn:
                connectedchat = await QUEEN_PTB.bot.getChat(conn)
        ...
        if (isadmin) or (isallow and ismember) or (user.id in SUDO_USERS):
            if connection_status := sql.connect(
                ...
                chat_name = await QUEEN_PTB.bot.getChat(chat.id).title
                send_message(
                    update.effective_message,
                    f"Successfully connected to *{chat_name}*.",
                    parse_mode=ParseMode.MARKDOWN,
...
QUEEN_PTB.add_handler(CommandHandler("connect", connect_chat, block=False))
