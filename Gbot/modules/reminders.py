...
from Gbot import SUDO_USERS, QUEEN_PTB
...
from Gbot.modules.ping import get_readable_time
from Gbot.modules.sql import remind_sql as sql
...
async def reminders(update: Update,
    ...
    if len(text) > 4096:
        ...
        with io.BytesIO(str.encode(text)) as file:
            ...
            await QUEEN_PTB.bot.send_document(
...
async def check_reminds(update: Update,
    ...
    while True:
        ...
        if t in sql.REMINDERS:
            ...
            for a in r:
                try:
                    user = await QUEEN_PTB.bot.get_chat(a["user_id"])
...
QUEEN_PTB.add_handler(CommandHandler(["remind", "reminder"], remind, block=False))
QUEEN_PTB.add_handler(CommandHandler(["reminds", "reminders"], reminders, block=False))
QUEEN_PTB.add_handler(CommandHandler("clearreminder", clearreminder, block=False))
QUEEN_PTB.add_handler(CommandHandler("clearallreminders",
