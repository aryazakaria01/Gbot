...
from Gbot import QUEEN_PTB, LOGGER, OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
from Gbot.__main__ import DATA_IMPORT

from Gbot.modules.helper_funcs.anonymous import user_admin
import Gbot.modules.sql.rules_sql as rulessql
import Gbot.modules.sql.blacklist_sql as blacklistsql
from Gbot.modules.sql import disable_sql as disabledsql
import Gbot.modules.sql.welcome_sql as welcsql
import Gbot.modules.sql.locks_sql as locksql
...
async def export_data(update: Update,
    ...
    with open("Gbot{}Backup".format(chat_id), "w") as f:
    ...
    await context.bot.sendDocument(
        current_chat_id,
        document=open("Gbot{}Backup".format(chat_id), "rb"),
        caption=
        "*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Queen-Robot-Backup` was specially made for notes."
        .format(
            chat.title,
            chat_id,
            tgl,
    ...
    os.remove("Gbot{}Backup".format(chat_id))  # Cleaning file
...
QUEEN_PTB.add_handler(CommandHandler("import", import_data, block=False))
QUEEN_PTB.add_handler(CommandHandler("export", export_data, block=False))
