...
import html
import re
import Gbot.modules.sql.rules_sql as rules_sql

from typing import Optional

from Gbot import BAN_STICKER, DEV_USERS, OWNER_ID, SUDO_USERS, WHITELIST_USERS, QUEEN_PTB ,BOT_USERNAME
...
from Gbot.modules.helper_funcs.string_handling import split_quotes
from Gbot.modules.log_channel import loggable
from Gbot.modules.sql import warns_sql as sql
from Gbot.modules.sql.approve_sql import is_approved
from Gbot.modules.helper_funcs.admin_status import user_admin_check, bot_admin_check, AdminPerms, bot_is_admin, user_is_admin
...
@bot_admin_check(AdminPerms.CAN_RESTRICT_MEMBERS)
# QUEEN_PTB handler stop - do not async
...
async def add_warn_filter(update: Update,
    ...
    for handler in QUEEN_PTB.handlers.get(WARN_HANDLER_GROUP, []):
        if handler.filters == (keyword, chat.id):
            QUEEN_PTB.remove_handler(handler, WARN_HANDLER_GROUP)
...
QUEEN_PTB.add_handler(
    CommandHandler(["resetwarn", "resetwarns"],
                   ...
                   filters=filters.ChatType.GROUPS,
                   block=False))
#QUEEN_PTB.add_handler(CommandHandler(["rmwarn", "unwarn"], remove_warns, filters=filters.ChatType.GROUPS, block=False)
QUEEN_PTB.add_handler(CallbackQueryHandler(button, pattern=r"rm_warn", block=False))
QUEEN_PTB.add_handler(
