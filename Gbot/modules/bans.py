...
from Gbot import (BAN_STICKER, DEV_USERS, ERROR_LOGS, SUDO_USERS,
                            OWNER_ID, WHITELIST_USERS, LOGGER, QUEEN_PTB)

from Gbot.modules.helper_funcs.chat_status import connection_status, dev_plus, is_user_admin
from Gbot.modules.helper_funcs.extraction import extract_user_and_text
from Gbot.modules.helper_funcs.string_handling import extract_time
from Gbot.modules.disable import DisableAbleCommandHandler
from Gbot.modules.log_channel import loggable, gloggable
...
QUEEN_PTB.add_handler(CommandHandler(['ban', 'dban', 'sban', 'dsban'], ban, block=False))
QUEEN_PTB.add_handler(CommandHandler(["tban"], temp_ban, block=False))
QUEEN_PTB.add_handler(CommandHandler(["kick", "punch"], kick, block=False))
QUEEN_PTB.add_handler(CommandHandler("unban", unban, block=False))
QUEEN_PTB.add_handler(CommandHandler(['selfunban', 'roar'], selfunban, block=False))
QUEEN_PTB.add_handler(CallbackQueryHandler(unbanb_btn, pattern=r"unbanb_"))
QUEEN_PTB.add_handler(DisableAbleCommandHandler(["kickme", "punchme"], kickme, filters=filters.ChatType.GROUPS, block=False))
QUEEN_PTB.add_handler(CommandHandler("snipe", snipe, filters=filters.User(SUDO_USERS), block=False))
QUEEN_PTB.add_handler(CommandHandler("banme", banme, block=False))
