	...
	def wrapper(func):
		...
		async def wrapped(update: Update, context: Ctx, *args, **kwargs):
			...
			try:  # try to get from cache
				...
				bot_member = QUEEN_PTB.bot.getChatMember(chat.id, bot_id)
...
async def get_mem_from_cache(user_id: int, chat_id: int) -> ChatMember:
	with RLOCK:
		try:
			...
			admins = await QUEEN_PTB.bot.getChatAdministrators(chat_id)
...
QUEEN_PTB.add_handler(CBHandler(perm_callback_check, pattern = "anonCB", block=False))
