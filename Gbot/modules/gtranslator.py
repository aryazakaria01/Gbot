...
async def gtts(update: Update, context: CallbackContext) -> None:
    ...
    try:
        tts = gTTS(reply)
        tts.save("Queen.mp3")
        with open("Queen.mp3", "rb") as speech:
            msg.reply_audio(speech)
    finally:
        if os.path.isfile("Queen.mp3"):
            os.remove("Queen.mp3")
...
QUEEN_PTB.add_handler(DisableAbleCommandHandler(["tr", "tl"], translate, block=False))
QUEEN_PTB.add_handler(
    DisableAbleCommandHandler(["langs", "lang"], languages, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("tts", gtts, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("splcheck", spellcheck, block=False))
