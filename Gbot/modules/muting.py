...
from Gbot.modules.helper_funcs.chat_status import is_user_admin, connection_status
...
async def temp_nomedia(update: Update,
    ...
    if conn is not False:
        chatD = await QUEEN_PTB.bot.getChat(conn)
...
QUEEN_PTB.add_handler(CommandHandler("mute", mute, block=False))
QUEEN_PTB.add_handler(CommandHandler("unmute", unmute, block=False))
QUEEN_PTB.add_handler(CommandHandler(['tmute', 'tempmute'], temp_mute, block=False))
...
QUEEN_PTB.add_handler(CallbackQueryHandler(button, pattern=r"cb_unmute", block=False))
