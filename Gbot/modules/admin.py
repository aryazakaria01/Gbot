...
import contextlib
import requests
import Gbot.modules.sql.pin_sql as sql
...
from telegram.ext import CallbackContext, CallbackQueryHandler, CommandHandler, filters as PTB_Queen_Filters
...
from Gbot import SUDO_USERS, TOKEN, QUEEN_PTB, LOGGER, pgram, telethn
from Gbot.modules.helper_funcs.msg_types import get_message_type
from Gbot.modules.helper_funcs.anonymous import user_admin
from Gbot.modules.disable import DisableAbleCommandHandler
from Gbot.modules.connection import connected
from Gbot.modules.users import build_keyboard_alternate
from Gbot.modules.helper_funcs.admin_status import user_admin_check, bot_admin_check, AdminPerms
...
from Gbot.events import register as CUTIEPII

ENUM_FUNC_MAP = {
    'Types.TEXT': QUEEN_PTB.bot.send_message,
    'Types.BUTTON_TEXT': QUEEN_PTB.bot.send_message,
    'Types.STICKER': QUEEN_PTB.bot.send_sticker,
    'Types.DOCUMENT': QUEEN_PTB.bot.send_document,
    'Types.PHOTO': QUEEN_PTB.bot.send_photo,
    'Types.AUDIO': QUEEN_PTB.bot.send_audio,
    'Types.VOICE': QUEEN_PTB.bot.send_voice,
    'Types.VIDEO': QUEEN_PTB.bot.send_video,
}
    ...
    try:
        if await is_user_admin(update, user.id):
            if mode == "demote":
                ...
                await QUEEN_PTB.bot.promote_chat_member(
                    chat.id,
                    user_id,
...
async def permanent_pin_set(update: Update,
    ...
    if conn:
        chat = QUEEN_PTB.bot.getChat(conn)
        chat_id = conn
        chat_name = QUEEN_PTB.bot.getChat(conn).title
        if not args:
...
QUEEN_PTB.add_handler(
    CommandHandler("setdesc",
                   set_desc,
                   filters=PTB_Queen_Filters.ChatType.GROUPS, block=False))
QUEEN_PTB.add_handler(
...
QUEEN_PTB.add_handler(
    CommandHandler("pin", pin, filters=PTB_Queen_Filters.ChatType.GROUPS, block=False))
QUEEN_PTB.add_handler(
...
QUEEN_PTB.add_handler(DisableAbleCommandHandler("invitelink", invite, block=False))

QUEEN_PTB.add_handler(DisableAbleCommandHandler("promote", promote, block=False))
QUEEN_PTB.add_handler(
    CallbackQueryHandler(promote_button, pattern=r"admin_", block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("fullpromote", fullpromote, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("lowpromote", lowpromote, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("midpromote", midpromote, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("setanon", promoteanon, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("demote", demote, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("middemote", middemote, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("unsetanon", demoteanon, block=False))

QUEEN_PTB.add_handler(CommandHandler("title", set_title, block=False))
QUEEN_PTB.add_handler(
