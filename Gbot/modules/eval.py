...
from Gbot import DEV_USERS, LOGGER, QUEEN_PTB
from Gbot import pgram
from Gbot.modules.helper_funcs.chat_status import dev_plus

Queen_PYRO_Eval = filters.command(["eval", "e"])
namespaces = {}
...
async def do(func, bot, update):
    ...
    with open(
            os.path.join(os.getcwd(),
                         "Gbot/modules/helper_funcs/temp.txt"),
            "w",
    ) as temp:
        temp.write(body)
    stdout = io.StringIO()
...
@pgram.on_message(Queen_PYRO_Eval & filters.user(DEV_USERS) &
                  (~filters.forwarded) & (~filters.via_bot))
@pgram.on_edited_message(Queen_PYRO_Eval)
...
QUEEN_PTB.add_handler(CommandHandler(("x", "ex", "exe", "py"), execute, block=False))
QUEEN_PTB.add_handler(CommandHandler("clearlocals", clear, block=False))

__mod_name__ = "[✨ ᴇᴠᴀʟ-ᴍᴏᴅᴇ ✨]"
