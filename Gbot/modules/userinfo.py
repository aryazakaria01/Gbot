...
from Gbot.utils.sections import section
from Gbot import (
    DEV_USERS,
    OWNER_ID,
    SUDO_USERS,
...
from Gbot.__main__ import TOKEN, USER_INFO
import Gbot.modules.sql.userinfo_sql as sql
from Gbot.modules.helper_funcs.misc import delete
from Gbot.modules.disable import DisableAbleCommandHandler
from Gbot.modules.sql.global_bans_sql import is_user_gbanned
from Gbot.modules.redis.afk_redis import is_user_afk, afk_reason
from Gbot.modules.sql.users_sql import get_user_num_chats
from Gbot.modules.sql.clear_cmd_sql import get_clearcmd
from Gbot.modules.helper_funcs.extraction import extract_user

Queen_PYRO_Whois = filters.command("whois")
...
@pgram.on_message(Queen_PYRO_Whois)
@pgram.on_edited_message(Queen_PYRO_Whois)
...
def hpmanager(user):
    ...
    if not is_user_gbanned(user.id):
        ...
        try:
            QUEEN_PTB.bot.get_user_profile_photos(user.id).photos[0][-1]
        except IndexError:
                ...
                delmsg = message.reply_document(
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        [
                            InlineKeyboardButton(
                                "Health",
                                url="https://t.me/LOGI_CHANNEL/70"),
                            InlineKeyboardButton(
                                "Disaster",
                                url="https://t.me/LOGI_CHANNEL/70"),
                        ],
                ...
                delmsg = await message.reply_text(
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        [
                            InlineKeyboardButton(
                                "Health",
                                url="hhttps://t.me/LOGI_CHANNEL/70"),
                            InlineKeyboardButton(
                                "Disaster",
                                url="https://t.me/LOGI_CHANNEL/70"),
                        ],
...
*What is that health thingy?*
 Come and see [HP System explained](https://t.me/LOGI_CHANNEL/70)

"""

QUEEN_PTB.add_handler(DisableAbleCommandHandler("setbio", set_about_bio, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("bio", about_bio, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("id", get_id, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("gifid", gifid, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("info", info, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("setme", set_about_me, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("me", about_me, block=False))
