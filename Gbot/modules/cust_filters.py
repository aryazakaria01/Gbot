...
from Gbot import QUEEN_PTB, SUDO_USERS, LOGGER
from Gbot.modules.helper_funcs.msg_types import get_filter_type
from Gbot.modules.helper_funcs.misc import build_keyboard_parser
from Gbot.modules.helper_funcs.string_handling import escape_invalid_curly_brackets
from Gbot.modules.helper_funcs.string_handling import (
    split_quotes,
    button_markdown_parser,
    markdown_to_html,
)
from Gbot.modules.log_channel import loggable
from Gbot.modules.sql import cust_filters_sql as sql
from Gbot.modules.connection import connected
from Gbot.modules.disable import DisableAbleCommandHandler
from Gbot.modules.helper_funcs.alternate import send_message
from Gbot.modules.helper_funcs.extraction import extract_text
...
ENUM_FUNC_MAP = {
    sql.Types.TEXT.value: QUEEN_PTB.bot.send_message,
    sql.Types.BUTTON_TEXT.value: QUEEN_PTB.bot.send_message,
    sql.Types.STICKER.value: QUEEN_PTB.bot.send_sticker,
    sql.Types.DOCUMENT.value: QUEEN_PTB.bot.send_document,
    sql.Types.PHOTO.value: QUEEN_PTB.bot.send_photo,
    sql.Types.AUDIO.value: QUEEN_PTB.bot.send_audio,
    sql.Types.VOICE.value: QUEEN_PTB.bot.send_voice,
    sql.Types.VIDEO.value: QUEEN_PTB.bot.send_video,
    # sql.Types.VIDEO_NOTE.value: QUEEN_PTB.bot.send_video_note
}
...
# NOT ASYNC BECAUSE QUEEN_PTB HANDLER RAISED
...
async def filters(update, context) -> None:  # sourcery no-metrics
    ...
    for handler in QUEEN_PTB.handlers.get(HANDLER_GROUP, []):
        if handler.filters == (keyword, chat_id):
            QUEEN_PTB.remove_handler(handler, HANDLER_GROUP)
...
# NOT ASYNC BECAUSE QUEEN_PTB HANDLER RAISE
@user_admin_check(AdminPerms.CAN_CHANGE_INFO)
@loggable
            ...
            if filt.reply == "there is should be a new reply":
                ...
                if filt.file_type in (sql.Types.BUTTON_TEXT, sql.Types.TEXT):
                    try:
                        ...
                        filt.file_type] == QUEEN_PTB.bot.send_sticker:
...
QUEEN_PTB.add_handler(DisableAbleCommandHandler("filter", filters))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("stop", stop_filter))
QUEEN_PTB.add_handler(DisableAbleCommandHandler(
    ["removeallfilters", "stopall"], rmall_filters, filters=PTB_Queen_Filters.ChatType.GROUPS, block=False))
QUEEN_PTB.add_handler(CallbackQueryHandler(
    rmall_callback, pattern=r"filters_.*", block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler(
    "filters", list_handlers, admin_ok=True, block=False))
QUEEN_PTB.add_handler(MessageHandler(PTB_Queen_Filters.TEXT & ~PTB_Queen_Filters.UpdateType.EDITED_MESSAGE, reply_filter, block=False))
