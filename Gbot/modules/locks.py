...
import asyncio
import ast
import Gbot.modules.sql.locks_sql as sql
...
async def build_lock_message(chat_id):
    ...
    permissions = await QUEEN_PTB.bot.get_chat(chat_id).permissions
    permslist = [
...
async def list_locks(update: Update,
    ...
    if conn:
        chat = await QUEEN_PTB.bot.getChat(conn)
...
QUEEN_PTB.add_handler(DisableAbleCommandHandler("locktypes", locktypes, block=False))
QUEEN_PTB.add_handler(CommandHandler("lock",
                                        lock, block=False))  # , filters=Filters.group)
QUEEN_PTB.add_handler(CommandHandler("unlock",
                                        unlock, block=False))  # , filters=Filters.group)
QUEEN_PTB.add_handler(CommandHandler(
