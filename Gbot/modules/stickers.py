...
from Gbot import REDIS, QUEEN_PTB, telethn, LOGGER
from Gbot.events import register as Queen
...
async def makepack_internal(context,
    ...
    try:
        ...
        if e.message == "Sticker set name is already occupied":
            ...
            await msg.reply_text(
                "Qᴜᴇᴇɴ ʙᴏᴛ 愛 was blocked by you.",
...
async def drawText(image_path, text):
    ...
    if os.name == "nt":
        ...
        fnt = "./Gbot/resources/ArmWrestler.ttf"
...
QUEEN_PTB.add_handler(DisableAbleCommandHandler("stickerid", stickerid, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("getsticker", getsticker, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("kang", kang, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("delsticker", delsticker, block=False))
QUEEN_PTB.add_handler(
    DisableAbleCommandHandler(["addfsticker", "afs"], add_fvrtsticker, block=False))
QUEEN_PTB.add_handler(
    DisableAbleCommandHandler(["removefsticker", "rfs"], remove_fvrtsticker, block=False))
QUEEN_PTB.add_handler(
    DisableAbleCommandHandler(["myfsticker", "mfs"], list_fvrtsticker, block=False))
QUEEN_PTB.add_handler(DisableAbleCommandHandler("stickers", cb_sticker, block=False))
QUEEN_PTB.add_handler(CallbackQueryHandler(cbs_callback, pattern="cbs_", block=False))
